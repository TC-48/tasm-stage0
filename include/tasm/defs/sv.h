...
#define SV(STRING_LITERAL) ((StringView) { .data = (STRING_LITERAL ""), .len = sizeof(STRING_LITERAL) - 1 })
...
static inline ulong sv_print(StringView sv, FILE* out) {
...
static inline StringView sv_from_data_and_len(const char* data, usize len) {
...
static inline StringView sv_slice(StringView sv, usize start, usize end) {
    ...
    return (StringView) { .data = sv.data + start, .len = end - start };
}

static inline StringView sv_window(StringView sv, usize start, usize len) {
    if (start >= sv.len) return SV_NULL;
    if (start + len > sv.len) len = sv.len - start;

    return (StringView) { .data = sv.data + start, .len = len };
}
